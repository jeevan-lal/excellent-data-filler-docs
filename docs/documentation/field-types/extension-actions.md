...
Extension Actions are powerful field types that allow you to interact with Excel (.xlsx) data directly from your forms. These actions enable you to manipulate Excel entries, modify data, and perform bulk operations on your spreadsheet data.
...
- **Row Freezing**: Freeze the current Excel row for a specified duration
- **Row Unfreezing**: Unfreeze frozen Excel rows
- **Data Manipulation**: Set, get, and modify Excel column values
- **Data Management**: Mark entries as saved, delete entries, and refresh data
...
### Field Options {#excel-row-field-options}
...
- **Delete**: Remove the entire row from the Excel (.xlsx) file
...
This field type applies actions to all entries in your Excel (.xlsx) file.

:::warning Warning
This operation can take a significant amount of time when working with large datasets, as it processes every row in the Excel (.xlsx) file.
:::

### Field Options {#excel-rows-field-options}
...
## Excel Row Freeze {#excel-row-freeze}
...
### Field Options {#excel-row-freeze-field-options}
...
- **Temp Lock Row**: Freeze a row for `2` minutes to avoid duplicate processing in concurrent execution environments.
...
### Field Options {#excel-rows-unfreeze-field-options}
...
## Set Excel Column {#set-excel-column}

Sets or updates the value of a specific column in an Excel row.

### Description {#set-excel-column-description}

This field type allows you to write data to a specific cell in your Excel (.xlsx) file by specifying the row number and column/field name.

### Field Options {#set-excel-column-field-options}
...
This field type reads data from a specific cell in your Excel (.xlsx) file and can be used to populate form fields or perform conditional logic based on existing data.

### Field Options {#get-excel-column-field-options}
...
- Load configuration data from Excel
...
### Field Options {#modify-excel-column-field-options}
...
This field type forces the extension to reload the Excel (.xlsx) file data, ensuring that any external changes to the file are reflected in the current session.

### Field Options {#refresh-excel-data-field-options}
...
- After external modifications to the Excel (.xlsx) file
