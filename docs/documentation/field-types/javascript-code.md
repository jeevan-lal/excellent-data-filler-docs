...
Insert structured log messages during JavaScript execution. These logs are simultaneously sent to both the **browser console window** and the built-in **[Extension Logs Viewer](/documentation/logs)**.

| Function | Description | Parameters |
| -------- | ----------- | ---------- |
| **$fns.log.success** | Logs a success message with optional contextual data | `message: String`, `data?: any` |
| **$fns.log.error** | Logs an error message with optional contextual data | `message: String`, `data?: any` |
| **$fns.log.warning** | Logs a warning message with optional contextual data | `message: String`, `data?: any` |
...
::: info Note
Log entries generated by these functions are displayed in:
- **Browser Console Window**: Developer Tools Console (`F12` or `Ctrl + Shift + J`).
- **[Extension Logs Viewer](/documentation/logs)**: Extension dashboard log stream and status bar logs popover.
:::
